...
pygame.display.set_caption('SAVE KUSH')
...
class Kush:
    ...
    def __init__(self):
        self.kush_img = pygame.image.load('kush2.png')
        self.kush_img_rect = self.kush_img.get_rect()
        self.kush_img_rect.left = 20
        self.kush_img_rect.top = WINDOW_HEIGHT/2 - 100
        self.down = True
        self.up = False

    def update(self):
        canvas.blit(self.kush_img, self.kush_img_rect)
        if self.kush_img_rect.top <= cactus_img_rect.bottom:
            game_over()
            if SCORE > self.kush_score:
                self.kush_score = SCORE
        if self.kush_img_rect.bottom >= fire_img_rect.top:
            game_over()
            if SCORE > self.kush_score:
                self.kush_score = SCORE
        if self.up:
                self.kush_img_rect.top -= 10
        if self.down:
                self.kush_img_rect.bottom += 10

def game_over():
    pygame.mixer.music.stop()
    music = pygame.mixer.Sound('kush_dies.wav')
    music.play()
...
def game_loop():
    while True:
        ...
        while True:
            ...
            level_font_rect.center = (800, cactus_img_rect.bottom + score_font_rect.height/2)
            ...
            kush.update()
            for f in flames_list:
                if f.flames_img_rect.colliderect(kush.kush_img_rect):
                    game_over()
                    if SCORE > kush.kush_score:
                        kush.kush_score = SCORE
            pygame.display.update()
            CLOCK.tick(FPS)
